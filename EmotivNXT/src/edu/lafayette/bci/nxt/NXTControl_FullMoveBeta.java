 ...
 *
 */
public class NXTControl_FullMoveBeta {
	 ...
	 */
	public NXTControl_FullMoveBeta() {
	 ...
	 * The robot will continue to move in its current direction while turning.
	 * If the robot is moving before it starts to turn, then the
	 * 
	 * @param speed The percentage value of the maximum speed at which the robot should turn.
	 */
	public void turnLeft(double speed, boolean moving, boolean forward) {
		if (!moving) {
			robot.setRotateSpeed(limiter(speed, 0, 1) * robot.getMaxRotateSpeed());
			robot.rotateLeft();
		} else if (forward) {
			robot.steer(limiter(speed,0,1)*200);
		} else if (!forward) {
			robot.steerBackward(limiter(speed,0,1)*200);
		}
	}
	 ...
	 * @param speed The percentage value of the maximum speed at which the robot should turn.
	 */
	public void turnRight(double speed, boolean moving, boolean forward) {
		if (!moving) {
			robot.setRotateSpeed(limiter(speed, 0, 1) * robot.getMaxRotateSpeed());
			robot.rotateRight();
		} else if (forward) {
