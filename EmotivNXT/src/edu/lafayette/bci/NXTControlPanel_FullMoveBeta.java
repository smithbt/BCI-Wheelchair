...
import edu.lafayette.bci.nxt.NXTControl_FullMoveBeta;

/**
 * This is the main class for the NXTControlPanel_FullMoveBeta.  It reads data
 ...
 * the NXT commands.
 * 
 * NOTE: This class is a beta class for trying to implement turning as part of 
 * forward and backward movement. It is not meant to be used as is.
 * 
 * @author Brandon T. Smith
 */
public class NXTControlPanel_FullMoveBeta implements EmotivObserver, KeyListener,
		WatchdogObserver {
	...
	private static final double OCCIPITAL_THRES = 25.0; // Power threshold in (uV)^2
	...
	private static final double BLINK_THRES = 100.0; // Blink detection threshold, in uV
	...
	private NXTControl_FullMoveBeta nxt = null;

	// UI class
	private NXTControlPanelUI_FullMoveBeta ui = null;
	// Set standard bottom label
	private static final String STANDARD_BOTTOM_LABEL = "Start/Stop: Close Eyes, Toggle Forward/Back: Blink x5, Turn: Rotate Head (Reset Shift+C)";

	// Connection timer
	private Watchdog wd = null;

	// State variables
	private boolean isTurning = false; // Indicates if the nxt is turning or stopped
	private boolean moving = false; // True if the nxt is moving, false if stopped
	private boolean forward = true; // True if the nxt is (or will be) moving forward, false if moving back
	 ...
	 */
	public NXTControlPanel_FullMoveBeta() {
		
		// Create UI
		ui = new NXTControlPanelUI_FullMoveBeta();
		ui.setDirection(NXTControlPanelUI_FullMoveBeta.STOP);
		...
		nxt = new NXTControl_FullMoveBeta();
		wd = new Watchdog();
		...
		if (!ENABLE_BLINK) {
			...
			ui.setDirection(NXTControlPanelUI_FullMoveBeta.FORWARD);
		}
	...
	public void sensorsChanged(Emotiv e) {
		...
		if (frontalData[frontalData.length - 1].getY() == 1.0) {
			...
			if (moving && forward && !isTurning) {
				...
				ui.setDirection(NXTControlPanelUI_FullMoveBeta.BACKWARD);
			}
		...
		if (lastPoint.getY() == 1 && moving) {
			...
			if (forward) {
				nxt.forward(NXT_SPEED);
				ui.setDirection(ui.getDirection() | NXTControlPanelUI_FullMoveBeta.FORWARD);
			} else {
				nxt.backward(NXT_SPEED);
				ui.setDirection(ui.getDirection() | NXTControlPanelUI_FullMoveBeta.BACKWARD);
			}
		} else if (lastPoint.getY() == -1) {
			// Falling edge detected, reverse movement state
			moving = !moving;	
		}
	...
	public void gyrosChanged(Emotiv e) {
		...
		if (speed < 0) {
			nxt.turnLeft(speed * -1 / GYROX_POS_MAX, moving, forward);
			ui.setDirection(ui.getDirection() | NXTControlPanelUI_FullMoveBeta.LEFT);
			isTurning = true;
		} else if (speed != 0) {
			nxt.turnRight(speed / GYROX_POS_MAX, moving, forward);
			ui.setDirection(ui.getDirection() | NXTControlPanelUI_FullMoveBeta.RIGHT);
			isTurning = true;
		} else if (isTurning) {
			if (moving && forward) nxt.forward(NXT_SPEED);
			else if (moving && !forward) nxt.backward(NXT_SPEED);
			else if (!moving) nxt.stop();
			ui.setDirection(ui.getDirection() & ~NXTControlPanelUI_FullMoveBeta.LEFT & ~NXTControlPanelUI_FullMoveBeta.RIGHT);
			isTurning = false;
		}
	...
	public static void main(String[] args) {
		...
		new NXTControlPanel_FullMoveBeta();
